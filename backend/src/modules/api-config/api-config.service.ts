...
export class ApiConfigService {
  ...
  async create(dto: CreateApiConfigDto) {
    ...
    // 清除交易所实例缓存,使新配置生效
    this.exchangeService.clearCache(dto.environment);

    this.logger.log(`Created ApiConfig: ${dto.exchange}:${dto.environment}`);
  ...
  async update(id: string, dto: UpdateApiConfigDto) {
    ...
    this.exchangeService.clearCache(existing.environment);
   ...
   * 测试 API 连通性(不落库)
   */
  async testConnection(dto: TestApiConfigDto): Promise<{ success: boolean; message: string }> {
    const ok = await this.exchangeService.testConnection(
      dto.apiKey,
      dto.apiSecret,
      dto.environment,
    );
   ...
   * 测试已存储配置的连通性
   */
  async testStoredConfig(id: string): Promise<{ success: boolean; message: string }> {
    ...
    const ok = await this.exchangeService.testConnection(apiKey, apiSecret, config.environment);
    return {
      success: ok,
   ...
   */
  async testByEnvironment(environment: string): Promise<{ success: boolean; message: string }> {
    ...
    const apiKey = this.cryptoService.decrypt(config.apiKey);
    const apiSecret = this.cryptoService.decrypt(config.apiSecret);
    const ok = await this.exchangeService.testConnection(apiKey, apiSecret, environment);
    return {
      success: ok,
      message: ok ? '连接成功' : '连接失败,请检查 API Key/Secret',
    };
  }
  ...
  private maskSecrets(config: any) {
    return {
      ...config,
      apiKey: this.mask(config.apiKey),
      apiSecret: this.mask(config.apiSecret),
    };
  }
